...
export const ArrayVisualization = ({ array, animationState }: ArrayVisualizationProps) => {
  ...
  const getStyle = (index: number) => {
    ...
    if (animationState) {
      if (animationState.type === 'sorted') {
        bgColor = 'bg-green-500';
        borderColor = 'border-green-600';
        textColor = 'text-white';
      } else if (animationState.type === 'swapping') {
        if (index === animationState.stand || index === animationState.lowestIndex) {
          bgColor = 'bg-red-600';
        ...
        if (index === animationState.stand) {
          ...
          scale = 'scale-105';
        } else if (index === animationState.lowestIndex) {
    ...
    return { bgColor, borderColor, textColor, scale };
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-end justify-center gap-1 md:hidden min-h-[200px] px-2">
        {array.map((value, index) => {
          const { bgColor, borderColor, textColor, scale } = getStyle(index);
          ...
          return (
            ...
            >
              ...
              <div className="text-xs font-normal text-gray-500 mt-1">[{index}]</div>
            </div>
          );
        })}
      </div>
      ...
      <div className="hidden md:flex md:flex-wrap md:items-end md:justify-center md:gap-3 min-h-[200px]">
        {array.map((value, index) => {
          ...
          return (
            <div
              key={`${index}-${value}`}
              className={`relative w-16 flex items-center justify-center rounded-lg border-2 font-mono font-bold text-lg transition-all duration-300 ${bgColor} ${borderColor} ${textColor} ${scale} shadow-md`}
              style={{
